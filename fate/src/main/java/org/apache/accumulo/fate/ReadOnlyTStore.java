...
public interface ReadOnlyTStore<T> {
     ...
     *
     * Reserving a transaction id ensures that nothing else in-process interacting via the same instance will be
     * operating on that transaction id.
     *
     ...
     *
     * upon successful return the store now controls the referenced transaction id. caller should no longer interact
     * with it.
     *
     * @param tid
     *            transaction id, previously reserved.
     * @param deferTime
     *            time in millis to keep this transaction out of the pool used in the {@link #reserve() reserve} method.

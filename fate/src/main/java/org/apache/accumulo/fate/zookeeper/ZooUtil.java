...
public class ZooUtil {
    ...
    public static final RetryFactory DEFAULT_RETRY = Retry.builder().maxRetries(10).retryAfter(250, MILLISECONDS)
            .incrementBy(250, MILLISECONDS).maxWait(5, TimeUnit.SECONDS).logInterval(3, TimeUnit.MINUTES)
            .createFactory();
    ...
    public static boolean putPersistentData(ZooKeeperConnectionInfo info, String zPath, byte[] data, int version,
            NodeExistsPolicy policy) throws KeeperException, InterruptedException {
    ...
    public static boolean putPrivatePersistentData(ZooKeeperConnectionInfo info, String zPath, byte[] data,
            NodeExistsPolicy policy) throws KeeperException, InterruptedException {
    ...
    public static List<ACL> getACL(ZooKeeper zk, String zPath, Stat stat) throws KeeperException, InterruptedException {
        final Retry retry = RETRY_FACTORY.createRetry();
        while (true) {
            try {

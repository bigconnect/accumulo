...
public class AdminUtil<T> {
    ...
    public static class TransactionStatus {
        ...
        private TransactionStatus(Long tid, TStatus status, String debug, List<String> hlocks, List<String> wlocks,
                String top) {
        ...
        /**
         * @return This fate operations transaction id, formatted in the same way as FATE transactions are in the
         *         Accumulo logs.
         */
    ...
    public static class FateStatus {
        ...
        /**
         * Convert FATE transactions IDs in keys of map to format that used in printing and logging FATE transactions
         * ids. This is done so that if the map is printed, the output can be used to search Accumulo's logs.
         */
        ...
        private FateStatus(List<TransactionStatus> transactions, Map<Long, List<String>> danglingHeldLocks,
         ...
         *
         * @return map where keys are transaction ids and values are a list of table IDs and/or namespace IDs. The
         *         transaction IDs are in the same format as transaction IDs in the Accumulo logs.
         */
        ...
        /**
         * Get locks that are waiting to be acquired by non existent FATE transactions. These are table or namespace
         * locks.
         *
         * @return map where keys are transaction ids and values are a list of table IDs and/or namespace IDs. The
         *         transaction IDs are in the same format as transaction IDs in the Accumulo logs.
    ...
    /**
     * Returns a list of the FATE transactions, optionally filtered by transaction id and status. This method does not
     * process lock information, if lock information is desired, use
    ...
    /**
     * Get the FATE transaction status and lock information stored in zookeeper, optionally filtered by transaction id
     * and filter status.
     *
     * @param zs
     *            read-only zoostore
     ...
     */
    public FateStatus getStatus(ReadOnlyTStore<T> zs, IZooReader zk, String lockPath, Set<Long> filterTxid,
     ...
     */
    private void findLocks(IZooReader zk, final String lockPath, final Map<Long, List<String>> heldLocks,
            final Map<Long, List<String>> waitingLocks) throws KeeperException, InterruptedException {
        ...
        for (String id : lockedIds) {
            ...
            try {
                ...
                /*
                 * could be transient zk error. Log, but try to process rest of list rather than throwing exception here
                 */
     ...
     * @param filterTxid
     *            Optional. List of transactions to filter results - if null, all transactions are returned
     * @param filterStatus
     *            Optional. List of status types to filter results - if null, all transactions are returned.
     * @param heldLocks
     ...
     */
    private FateStatus getTransactionStatus(ReadOnlyTStore<T> zs, Set<Long> filterTxid, EnumSet<TStatus> filterStatus,
            Map<Long, List<String>> heldLocks, Map<Long, List<String>> waitingLocks) {
    ...
    public void print(ReadOnlyTStore<T> zs, IZooReader zk, String lockPath, Formatter fmt, Set<Long> filterTxid,
            EnumSet<TStatus> filterStatus) throws KeeperException, InterruptedException {
        ...
        for (TransactionStatus txStatus : fateStatus.getTransactions()) {
            fmt.format("txid: %s  status: %-18s  op: %-15s  locked: %-15s locking: %-15s top: %s%n", txStatus.getTxid(),
                    txStatus.getStatus(), txStatus.getDebug(), txStatus.getHeldLocks(), txStatus.getWaitingLocks(),
                    txStatus.getTop());
        }

...
public class Fate<T> {
    ...
    private static final EnumSet<TStatus> FINISHED_STATES = EnumSet.of(TStatus.FAILED, TStatus.SUCCESSFUL,
            TStatus.UNKNOWN);
     ...
     * <p>
     * Note: Users of this class should call {@link #startTransactionRunners(int)} to launch the worker threads after
     * creating a Fate object.
     */
    ...
    public String getReturn(long tid) {
        ...
        try {
            if (store.getStatus(tid) != TStatus.SUCCESSFUL)
                throw new IllegalStateException("Tried to get exception when transaction " + String.format("%016x", tid)

...
public class ZooCache {
    ...
    /**
     * Returns a ZooKeeper session. Calls should be made within run of ZooRunnable after caches are checked. This will
     * be performed at each retry of the run method. Calls to {@link #getZooKeeper()} should be made, ideally, after
     * cache checks since other threads may have succeeded when updating the cache. Doing this will ensure that we don't
     * pay the cost of retrieving a ZooKeeper session on each retry until we've ensured the caches aren't populated for
     * a given node.
     *
    ...
    private abstract class ZooRunnable<T> {
         ...
         *
         * Changes were made in ACCUMULO-4388 so that the run method no longer accepts Zookeeper as an argument, and
         * instead relies on the ZooRunnable implementation to call {@link #getZooKeeper()}. Performing the call to
         * retrieving a ZooKeeper Session after caches are checked has the benefit of limiting ZK connections and
         * blocking as a result of obtaining these sessions.
         *
        ...
        /**
         * Retry will attempt to call the run method. Run should make a call to {@link #getZooKeeper()} after checks to
         * cached information are made. This change, per ACCUMULO-4388 ensures that we don't create a ZooKeeper session
         * when information is cached, and access to ZooKeeper is unnecessary.
         *
    ...
    /**
     * Gets data at the given path, filling status information into the given <code>Stat</code> object. A watch is
    ...
    public byte[] get(final String zPath, final ZcStat status) {
        ZooRunnable<byte[]> zr = new ZooRunnable<byte[]>() {
            ...
            public byte[] run() throws KeeperException, InterruptedException {
                ...
                /*
                 * The following call to exists() is important, since we are caching that a node does not exist. Once
                 * the node comes into existence, it will be added to the cache. But this notification of a node coming
                 * into existence will only be given if exists() was previously called. If the call to exists() is
                 * bypassed and only getData() is called with a special case that looks for Code.NONODE in the
                 * KeeperException, then non-existence can not be cached.
                 */
                cacheWriteLock.lock();
                try {
                    final ZooKeeper zooKeeper = getZooKeeper();
    ...
    /**
     * Returns a monotonically increasing count of the number of time the cache was updated. If the count is the same,
     * then it means cache did not change.
     */

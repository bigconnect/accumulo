...
public class AddressUtil {
    ...
    /**
     * Fetch the security value that determines how long DNS failures are cached. Looks up the security property
     * 'networkaddress.cache.negative.ttl'. Should that fail returns the default value used in the Oracle JVM 1.4+,
     * which is 10 seconds.
     *
    ...
    static public int getAddressCacheNegativeTtl(UnknownHostException originalException) {
        ...
        if (-1 == negativeTtl) {
            log.error("JVM negative DNS repsonse cache TTL is set to 'forever' and host lookup failed. "
